	...
	class JsonDeserializer
	{
		...
		private:
			void remove_spaces(const std::string &str)
			{
				...
				{
					if ((!isspace(str[i]) or is_string) and str[i] != '\n')
			...
			Json load_primitive_type()
			{
				...
				if (m_data.size() - offset >= 4 and memcmp(string_to_load, "null", 4) == 0)
				{
					offset += 4;
					return Json();
				}
				if (m_data.size() - offset >= 4 and memcmp(string_to_load, "true", 4) == 0)
				{
					offset += 4;
					return Json(true);
				}
				if (m_data.size() - offset >= 5 and memcmp(string_to_load, "false", 5) == 0)
				{
					offset += 5;
					return Json(false);
				}
...
namespace avocado
{
	...
	Json::Json(const std::initializer_list<Json> &list)
	{
		...
		{
			return element.isArray() and element.size() == 2 && element[0].isString();
		});
	...
	{
		if (idx < 0)
			throw IllegalArgument(METHOD_NAME, "idx", "must not be negative", std::to_string(idx));
	...
	void Json::append(const Json &other)
	{
		...
		if (not other.isObject())
			throw JsonTypeError(METHOD_NAME, other.storedType());
