...
//			}
		...
		public:
			...
			void set(avConvolutionMode_t mode, int nbDims, const int strides[], const int padding[], const int dilation[], int groups, const void *paddingValue)
			{
		...
		namespace internal
		{
			...
			inline DescriptorPool<ContextDescriptor>& getPool()
			{
				thread_local DescriptorPool<ContextDescriptor> result = []()
				{
					...
					tmp.create(i, true); // reserve descriptors for default contextes
						return tmp;
					}();
				return result;
			}
